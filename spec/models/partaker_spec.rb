...
describe Partaker, type: :model do
  describe 'associations' do
    ...
    it { is_expected.to belong_to(:trip_membership) }
    it { is_expected.to have_db_index(:expense_id) }
    it { is_expected.to have_db_index(:trip_membership_id) }
  end

  describe 'delegation' do
    it { is_expected.to delegate_method(:member).to(:trip_membership) }
